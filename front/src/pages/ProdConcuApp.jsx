...
export default function ProductsPage() {
  ...
  const fetchProducts = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${URL}/api/productConcu/appareillage `, {
        credentials: "include",
      });
  ...
  const confirmDelete = async () => {
    ...
    await fetch(
      `${URL}/api/productConcu/appareillage/${deleteProductTarget.ID}`,
      {
        method: "DELETE",
        credentials: "include",
      }
  ...
  const handleSubmit = async () => {
    if (selectedProduct) {
      // Update
      await fetch(
        `${URL}/api/productConcu/appareillage/${selectedProduct.ID}`,
        {
          method: "PUT",
      ...
      await fetch(`${URL}/api/productConcu/appareillage`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify(form),
    ...
    <div className="p-6 space-y-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold">
          Gestion des Produits Concurrents Appareillages
        </h1>
