...
export default function ProductsPage() {
  ...
  const fetchProducts = async () => {
    setLoading(true);
    try {
      const res = await fetch("http://localhost:3000/api/product/accessoire", {
        credentials: "include",
      });
  ...
  const confirmDelete = async () => {
    ...
    await fetch(
      `http://localhost:3000/api/product/accessoire/${deleteProductTarget.id}`,
      {
        method: "DELETE",
        credentials: "include",
      }
  ...
  const handleSubmit = async () => {
    if (selectedProduct) {
      // Update
      await fetch(
        `http://localhost:3000/api/product/accessoire/${selectedProduct.id}`,
        {
          method: "PUT",
      ...
      await fetch("http://localhost:3000/api/product/accessoire", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify(form),
