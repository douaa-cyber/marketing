const ProdConcurrentDisjoncteur = require("../../model/ProdConcurrentDisjoncteur");

const getAllProdConcurrentDisjoncteur = async (req, res) => {
  try {
    const items = await ProdConcurrentDisjoncteur.findAll();
    res.status(200).json(items);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};

const getProdConcurrentDisjoncteurById = async (req, res) => {
  try {
    const item = await ProdConcurrentDisjoncteur.findByPk(req.params.id);
    if (!item) {
      return res.status(404).json({ message: "Not found" });
    }
    res.status(200).json(item);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};

module.exports = {
  getAllProdConcurrentDisjoncteur,
  getProdConcurrentDisjoncteurById,
};
